...
#define OPTION_PREFIX_LEN strlen(OPTION_PREFIX)
  ...
  {
    const internal::MuxMapperList &muxers = internal::GetRegisteredMultiplexMappers();
  ...
  bool RGBMatrix::Options::Validate(std::string *err_in) const
  {
    ...
    {
      if ((!strchr(led_rgb_sequence, 'R') && !strchr(led_rgb_sequence, 'r')) || (!strchr(led_rgb_sequence, 'G') && !strchr(led_rgb_sequence, 'g')) || (!strchr(led_rgb_sequence, 'B') && !strchr(led_rgb_sequence, 'b')))
      {
